...
function ExportButton({ data }) {
  // disables if there is no data (columns or rows)
  const disabled = !data?.columns?.length || !data?.rows?.length;

  // generates and triggers download of PDF when button is clicked
  const handleClick = () => {
    if (!data?.columns?.length || !data?.rows?.length) return; // safety check: do nothing if no data
    const doc = new jsPDF();
    autoTable(doc, {
      head: [data.columns],
